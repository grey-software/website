      ...
      <nav class="hidden lg:flex lg:pt-1 xl:pt-0 mx-auto">
        <ul class="flex text-center">
          <li v-for="link in headerLinks" :key="link.slug" class="xl:px-4 lg:py-0 lg:px-2 py-2">
            <NuxtLink class="block p-2 font-medium uppercase hover:no-underline transition-colors duration-300 ease-linear" :to="{ name: link.routeName, params: { section: link.slug } }">
              {{ link.name }}
            </NuxtLink>
          </li>
        </ul>
